...
<head>
    ...
    <title>Expenses - Cashflow Diary</title>
...
<body class="bg-slate-50">
    <main class="p-4 md:p-10">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
            <div>
                <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Expenses</h1>
                <p class="text-slate-500 mt-1">Total: <span class="font-bold text-rose-600"><?php echo format_currency($total_expenses); ?></span></p>
            </div>
            
            <form class="flex flex-wrap gap-3 w-full md:w-auto">
                <input type="text" name="search" value="<?php echo e($search); ?>" placeholder="Search category, note..." class="px-4 py-2 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm">
        ...
        <div class="space-y-4">
            <?php if (empty($expenses)): ?>
                <div class="bg-white rounded-[2rem] p-20 text-center border border-slate-100 shadow-sm">
                    ...
                    <h3 class="text-xl font-bold text-slate-900 mb-2">No expenses found</h3>
                    <p class="text-slate-500">Add your first expense to see it here.</p>
                </div>
            <?php
else: ?>
                <div class="grid grid-cols-1 gap-4">
                    <?php foreach ($expenses as $expense): ?>
                        <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex items-center justify-between hover:border-rose-200 transition-all group">
                            ...
                            <div class="flex items-center space-x-6">
                                <span class="text-xl font-bold text-slate-900">-<?php echo format_currency($expense['amount']); ?></span>
    ...
    <a href="add.php" class="fixed bottom-10 right-10 w-16 h-16 bg-rose-600 text-white rounded-full flex items-center justify-center shadow-2xl shadow-rose-200 hover:bg-rose-700 transform hover:scale-110 active:scale-95 transition-all z-40">
