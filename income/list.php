...
<head>
    ...
    <title><?php echo __('income', 'Income'); ?> - <?php echo __('app_name', 'Cashflow Diary'); ?></title>
...
<body class="bg-slate-50">
    <main class="p-4 md:p-10">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
            <div>
                <h1 class="text-3xl font-bold text-slate-900 tracking-tight"><?php echo __('income', 'Income'); ?></h1>
                <p class="text-slate-500 mt-1"><?php echo __('total', 'Total'); ?>: <span class="font-bold text-emerald-600"><?php echo format_currency($total_income); ?></span></p>
            </div>
        ...
        <div class="space-y-4">
            <?php if (empty($income_records)): ?>
                <div class="bg-white rounded-[2rem] p-20 text-center border border-slate-100 shadow-sm">
                    ...
                    <h3 class="text-xl font-bold text-slate-900 mb-2"><?php echo __('no_income_found', 'No income found'); ?></h3>
                    <p class="text-slate-500"><?php echo __('add_your_first_income', 'Add your first income to see it here.'); ?></p>
                </div>
            <?php else: ?>
                <div class="grid grid-cols-1 gap-4">
                    <?php foreach ($income_records as $record): ?>
                        <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex items-center justify-between hover:border-emerald-200 transition-all group<?php echo get_lang_dir() === 'rtl' ? ' flex-row-reverse' : ''; ?>">
                            <div class="flex items-center <?php echo get_lang_dir() === 'rtl' ? 'space-x-reverse' : ''; ?> space-x-6">
                                ...
                                <div class="<?php echo get_lang_dir() === 'rtl' ? 'text-right' : ''; ?>">
                                    ...
                                    <div class="flex items-center <?php echo get_lang_dir() === 'rtl' ? 'space-x-reverse' : ''; ?> space-x-3 text-sm text-slate-500 mt-1">
                                        <span><?php echo format_date($record['date']); ?></span>
                                    </div>
                                    <?php if ($record['note']): ?>
                            ...
                            <div class="flex items-center <?php echo get_lang_dir() === 'rtl' ? 'space-x-reverse' : ''; ?> space-x-6">
                                <span class="text-xl font-bold text-emerald-600" dir="ltr">+<?php echo format_currency($record['amount']); ?></span>
    ...
    <a href="add.php" class="fixed <?php echo get_lang_dir() === 'rtl' ? 'left-10' : 'right-10'; ?> bottom-10 w-16 h-16 bg-emerald-600 text-white rounded-full flex items-center justify-center shadow-2xl shadow-emerald-200 hover:bg-emerald-700 transform hover:scale-110 active:scale-95 transition-all z-40">
