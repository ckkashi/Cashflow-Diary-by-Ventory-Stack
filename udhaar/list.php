...
<head>
    ...
    <title>Udhaar Ledger - Cashflow Diary</title>
...
<body class="bg-slate-50">
    <main class="p-4 md:p-10">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
            <div>
                <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Udhaar Ledger</h1>
                <p class="text-slate-500 mt-1">Manage your credit and debt transactions.</p>
            </div>
            
            <div class="flex flex-col md:flex-row gap-4 w-full md:w-auto items-end">
                <form class="flex gap-2 w-full md:w-auto">
                    <input type="text" name="search" value="<?php echo e($search); ?>" placeholder="Search contact..." class="px-4 py-2 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-amber-500 text-sm w-full">
                    <button type="submit" class="bg-amber-500 text-white px-6 py-2 rounded-xl font-semibold hover:bg-amber-600 transition-all text-sm">Search</button>
                </form>

                <div class="flex space-x-4 w-full md:w-auto">
                    <div class="flex-1 md:flex-none bg-emerald-50 p-4 px-6 rounded-2xl border border-emerald-100">
                        <p class="text-[10px] font-bold text-emerald-600 uppercase tracking-widest leading-none mb-1">Receivable</p>
                    ...
                    <div class="flex-1 md:flex-none bg-rose-50 p-4 px-6 rounded-2xl border border-rose-100">
                        <p class="text-[10px] font-bold text-rose-600 uppercase tracking-widest leading-none mb-1">Payable</p>
        ...
        <div class="space-y-4">
            <?php if (empty($contact_balances)): ?>
                <div class="bg-white rounded-[2rem] p-20 text-center border border-slate-100 shadow-sm">
                    ...
                    <h3 class="text-xl font-bold text-slate-900 mb-2">No Udhaar records yet</h3>
                    <p class="text-slate-500 mb-8">Start by adding a contact or a transaction.</p>
                    <div class="flex flex-col sm:flex-row justify-center gap-4">
                        <a href="../contacts/add.php" class="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100">Add Contact</a>
                        <a href="add.php" class="bg-amber-500 text-white px-8 py-4 rounded-2xl font-bold hover:bg-amber-600 transition-all shadow-lg shadow-amber-100">Add Transaction</a>
                    </div>
                </div>
            <?php else: ?>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <?php foreach ($contact_balances as $contact): ?>
                        <a href="view.php?id=<?php echo $contact['id']; ?>" class="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-200 hover:border-amber-200 hover:shadow-2xl hover:-translate-y-1 transition-all group flex flex-col justify-between h-56 relative overflow-hidden">
                            <!-- Background Accent -->
                            <div class="absolute -right-10 -top-10 w-40 h-40 <?php echo $contact['balance'] >= 0 ? 'bg-emerald-50' : 'bg-rose-50'; ?> rounded-full opacity-50 group-hover:scale-125 transition-transform duration-500"></div>
                            ...
                            <div class="relative">
                                <div class="flex items-center space-x-4 mb-4">
                                    ...
                                    <div class="overflow-hidden">
                                        ...
                                        <p class="text-xs text-slate-500 truncate"><?php echo e($contact['phone'] ?: 'No phone provided'); ?></p>
                                    </div>
                                </div>
                            </div>

                            <div class="relative pt-6 border-t border-slate-50 flex justify-between items-end">
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1"><?php echo $contact['balance'] >= 0 ? 'Receivable' : 'Payable'; ?></p>
    ...
    <a href="add.php" class="fixed bottom-10 right-10 w-16 h-16 bg-amber-500 text-white rounded-full flex items-center justify-center shadow-2xl shadow-amber-200 hover:bg-amber-600 transform hover:scale-110 active:scale-95 transition-all z-40">
