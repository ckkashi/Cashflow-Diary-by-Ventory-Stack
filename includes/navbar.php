...
<!-- Mobile Header -->
<div class="lg:hidden bg-white dark:bg-dark-card border-b dark:border-dark-border p-4 flex justify-between items-center sticky top-0 z-50">
    <div class="flex items-center space-x-2">
        <div class="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white font-bold">C</div>
        <span class="font-bold text-slate-800 dark:text-white">Cashflow Diary</span>
    </div>
    <button onclick="document.getElementById('sidebar').classList.toggle('-translate-x-full')" class="p-2 text-slate-500">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16m-7 6h7" /></svg>
    </button>
</div>

<!-- Sidebar -->
<aside id="sidebar" class="fixed inset-y-0 left-0 w-72 bg-white dark:bg-dark-card border-r dark:border-dark-border transform -translate-x-full lg:translate-x-0 transition-transform duration-300 z-50 flex flex-col">
    <!-- Branding -->
    <div class="p-8 border-b dark:border-dark-border">
        <div class="flex items-center space-x-3 mb-2">
            ...
            <div>
                <h1 class="font-bold text-slate-900 dark:text-white text-lg leading-tight">Cashflow Diary</h1>
    ...
    <div class="p-6">
        <div class="relative group">
            <button class="w-full flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border dark:border-dark-border hover:border-indigo-300 transition-all">
            ...
            <div class="absolute left-0 right-0 mt-2 bg-white dark:bg-dark-card border dark:border-dark-border rounded-2xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 overflow-hidden">
                <?php foreach ($businesses as $biz): ?>
                    <a href="../businesses/switch.php?id=<?php echo $biz['id']; ?>" class="flex items-center space-x-3 p-4 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
...
endforeach; ?>
    ...
    <div class="p-6 border-t dark:border-dark-border space-y-4">
        <!-- Dark Mode Toggle -->
        <button onclick="toggleDarkMode()" class="w-full flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border dark:border-dark-border text-slate-700 dark:text-slate-300 font-bold transition-all">
            <span class="flex items-center">
                <svg id="sun-icon" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3 hidden dark:block" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707M16.95 16.95l.707.707M7.05 7.05l.707.707M12 8a4 4 0 100 8 4 4 0 000-8z" /></svg>
                <svg id="moon-icon" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3 block dark:hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg>
                Dark Mode
            </span>
        ...
        <a href="../auth/logout.php" class="flex items-center space-x-4 px-4 py-4 rounded-2xl font-bold text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition-all">
...
<!-- Notification Hub -->
<div id="notification-container" class="fixed top-6 right-6 z-[200] space-y-3 px-4 w-full max-w-sm pointer-events-none"></div>
