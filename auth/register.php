    ...
    if ($name && $email && $password) {
        ...
        if ($stmt->fetch()) {
            ...
            if ($stmt->execute([$name, $email, $hashed_password])) {
                ...
                header('refresh:2;url=/cashflow/login');
            }
            else {
...
<head>
    ...
    <title><?php echo __('register', 'Register'); ?> - <?php echo __('app_name', 'Cashflow Diary'); ?></title>
...
<body class="bg-slate-50 hero-gradient min-h-screen flex items-center justify-center p-6 select-none">
    <div class="max-w-md w-full glass rounded-[3rem] shadow-2xl shadow-indigo-100/50 overflow-hidden p-10 md:p-12">
        <div class="text-center mb-10">
            ...
            <h1 class="text-3xl font-bold text-slate-900 tracking-tight"><?php echo __('create_account', 'Create Account'); ?></h1>
            <p class="text-slate-500 mt-2 font-medium"><?php echo __('join_thousands', 'Join Cashflow Diary today'); ?></p>
        </div>
        ...
        <form method="POST" class="space-y-6">
            <?php echo csrf_field(); ?>
            <div class="space-y-2">
                <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest px-1"><?php echo __('full_name', 'Full Name'); ?></label>
        ...
        <div class="mt-10 text-center">
            <p class="text-sm font-bold text-slate-400">
                ...
                <a href="/cashflow/login" class="text-indigo-600 hover:text-indigo-700 underline underline-offset-4 decoration-2 decoration-indigo-100 hover:decoration-indigo-600 transition-all"><?php echo __('sign_in', 'Sign In'); ?></a>
            </p>
        </div>
    </div>
</body>
